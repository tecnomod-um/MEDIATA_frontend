...
const Nodes = () => {
    ...
    useEffect(() => {
        const fetchNodes = async () => {
            try {
                ...
                const nodesChanged = newNodes.length !== nodes.length ||
                    newNodes.some((newNode, index) => {
                        const oldNode = nodes[index];
                        return !oldNode || Object.keys(newNode).some(key => newNode[key] !== oldNode[key]);
                    });
    ...
    return (
        <div className={NodesStyles.container}>
            {error && (
                <div className={`${NodesStyles.error} ${showError ? NodesStyles.fadeIn : NodesStyles.fadeOut}`}>
                    {error}
                </div>
            )}
