...
describe("parseCSV", () => {
  ...
  it("parses quoted values generated by Papa.unparse", () => {
    const csv = Papa.unparse([["CatCol", "A, B", "C"]], {
      quotes: true,
      newline: "\n",
    });
    ...
    expect(result).toEqual([{ column: "CatCol", values: ["A, B", "C"] }]);
  });

  it("skips empty lines when parsing exported element files", () => {
    const csv = 'col1,"A, B"\n\ncol2,val2';
    const result = parseCSV(csv);

    expect(result).toEqual([
      { column: "col1", values: ["A, B"] },
      { column: "col2", values: ["val2"] },
    ]);
  });
});
