...
function RdfParser() {
    ...
    const mergeColumnsData = (existingData, newData) => {
        ...
        newData.forEach(row => {
            const existingColumn = mergedData.find(item => item.column === row.column);
            if (existingColumn) {
    ...
    const handleSaveMappings = (groups, unionName, customValues, removeFromHierarchy) => {
        const updatedMappings = {
            ...
            [unionName]: {
                ...
                groups: groups.map(group => ({
                    column: group.column,
                    values: customValues.length > 0 ? customValues : group.values.map(value => ({
                        name: value,
        ...
        if (removeFromHierarchy) 
            groups.forEach(group => {  delete updatedMappings[group.column]; });
        ...
        return (
            <div className={RdfParserStyles.entryContainer}>
                ...
                <div className={RdfParserStyles.groupContainer}>
                    {mapping.groups.map((group, groupIndex) => (
                        <div key={groupIndex} className={RdfParserStyles.valueContainer}>
                            {group.values.map((valueObj, valIndex) => (
                                <div key={valIndex} className={RdfParserStyles.valueBox}>
                                    ...
                                    <div className={RdfParserStyles.mappings}>
                                        {valueObj.mapping.map((map, mapIndex) => (
                                            (map.groupColumn !== mappingKey || map.value !== valueObj.name) && (
                                                <div key={mapIndex} className={RdfParserStyles.mappingItem}>
                    ...
                    <div className={RdfParserStyles.columnsSection}>
                        <div className={RdfParserStyles.scrollableContainer}>
                            {columnsData.map((column, index) => (
                                <div
                                    key={index}
                                    className={RdfParserStyles.columnItem}
                                    draggable
