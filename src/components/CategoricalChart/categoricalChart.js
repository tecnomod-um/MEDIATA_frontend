...
const CategoricalChart = ({ feature, onClick, isSelected, onDoubleClick }) => {
  ...
  const labels = Object.keys(statistics).filter(
    (stat) => stat !== "MissingValues"
  );
  ...
  const selectedClass =
    chartSizeClass === categoricalChartStyles.large
      ? categoricalChartStyles.selectedLarge
      : categoricalChartStyles.selected;

  return (
    <div
      className={`${categoricalChartStyles.chartContainer} ${chartSizeClass} ${
