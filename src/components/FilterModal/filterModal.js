...
const FilterModal = ({
  ...
  const getCategoryOptions = (featureName) => {
    ...
    const selectedCategories = filterConditions[featureName]
      ? filterConditions[featureName].map((condition) => condition.value)
      : [];
  ...
  const formatSingleCriteria = (condition, featureName) => {
    if (Array.isArray(condition))
      return condition.map((item) => (
        <span key={item} className={FilterModalStyles.criteriaBox}>
          {item}
        </span>
      ));
    ...
    if (isDateFeature) {
      formattedValue = Array.isArray(value)
        ? value.map((date) => new Date(date).toLocaleDateString())
        : new Date(value).toLocaleDateString();
      formattedRange = Array.isArray(value)
        ? value.map((date) => new Date(date).toLocaleDateString())
  ...
  const getOptions = (dataStatistics) => {
    ...
    if (dataStatistics.categoricalFeatures)
      dataStatistics.categoricalFeatures.forEach((feature) => {
        options.push({
    ...
    if (dataStatistics.continuousFeatures)
      dataStatistics.continuousFeatures.forEach((feature) => {
        options.push({
          value: feature.featureName,
          label: feature.featureName,
        });
    ...
    if (dataStatistics.dateFeatures)
      dataStatistics.dateFeatures.forEach((feature) => {
        options.push({
          value: feature.featureName,
          label: feature.featureName,
        });
  ...
  const isAddFilterDisabled = () => {
    ...
    if (
      ...
      return continuousFilterType === "between"
        ? !minValue || !maxValue
        : !minValue;
    else if (
      dataStatistics.dateFeatures.some(
          ...
          {selectedFeature && (
            ...
            >
              {dataStatistics.categoricalFeatures.some(
                ...
                <div>
                  <label>
                    Only rows with the chosen categories will be shown
                  </label>
                  <Select
                    isMulti
                    ...
                    value={getCategoryOptions(selectedFeature).filter(
        ...
        {Object.keys(filterConditions).some((key) => filterConditions[key]) && (
          <div className={FilterModalStyles.filterSummary}>
            <TransitionGroup component="ul">
              {Object.entries(filterConditions).map(
                ([feature, criteria], featureIndex) =>
                  criteria && (
                    <CSSTransition
                      key={feature}
                      timeout={500}
