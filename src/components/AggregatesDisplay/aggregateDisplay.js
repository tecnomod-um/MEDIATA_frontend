...
const AggregateDisplay = ({ covariances, pearsonCorrelations, spearmanCorrelations, chiSquareTest, omittedFeatures }) => {
    ...
    const renderMatrix = (stats) => {
        if (isStatsEmpty(stats))
            return <div className={AggregateDisplayStyles.noData}>No aggregate data available</div>;
    ...
    const renderChiSquaredResults = () => {
        if (chiSquareTest.length === 0)
            return <div className={AggregateDisplayStyles.noData}>No Chi-Squared Tests available</div>;

        return (
            <div className={AggregateDisplayStyles.chiSquaredResultsContainer}>
                {chiSquareTest && chiSquareTest.map((test, index) => (
                    <div key={index}>
                        <strong>{`${test.category1} vs ${test.category2}`}: </strong>{typeof test.pvalue === 'number' ? test.pvalue.toFixed(4) : 'N/A'}
                    </div>
                ))}
            </div>
        );
    ...
    const renderOmittedFeatures = () => {
        if (omittedFeatures.length === 0)
            return <div className={AggregateDisplayStyles.noData}>No fields were omitted</div>;

        return (
            <div className={AggregateDisplayStyles.omittedFeaturesContainer}>
                {omittedFeatures.map((feature, index) => (
                    <div key={index}>
                        <strong>{feature.featureName}: </strong>{feature.reason} (Missing: {feature.percentMissing.toFixed(2)}%)
                    </div>
                ))}
            </div>
        );
    ...
    const isEmpty = isStatsEmpty(covariances) && isStatsEmpty(pearsonCorrelations) && isStatsEmpty(spearmanCorrelations);
    ...
    return (
        <div className={AggregateDisplayStyles.statsContainer}>
            ...
            {isEmpty && (<div className={AggregateDisplayStyles.noData}>No aggregated data available</div>)}
        </div>
    );
}
