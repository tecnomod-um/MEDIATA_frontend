...
jest.mock('rc-slider', () => ({
  __esModule: true,
  default: ({ onChange, onAfterChange }) => (
    <div data-testid="slider">
      <button onClick={() => { onChange([2, 4]); onAfterChange([2, 4]); }}>
        Simulate Slide
      </button>
    </div>
  ),
...
describe('<RangePicker />', () => {
  it('displays "All ranges have been mapped." when no available ranges', () => {
    ...
    expect(screen.getByText(/All ranges have been mapped\./i)).toBeInTheDocument();
  ...
  it('initializes to first available range, bumps overlapping, and calls onRangeChange on Set (integer)', () => {
    ...
    expect(screen.getByText(/Selected Range:\s*0\s*-\s*0/)).toBeInTheDocument();
    fireEvent.click(screen.getByText('Simulate Slide'));
    expect(screen.getByText(/Selected Range:\s*3\s*-\s*4/)).toBeInTheDocument();
  ...
  it('initializes date picker and calls onRangeChange on Set (date)', () => {
    ...
    expect(screen.getByText(new RegExp(`Selected Range:\\s*${iso}\\s*-\\s*${iso}`))).toBeInTheDocument();
    fireEvent.click(screen.getByText('Set'));
