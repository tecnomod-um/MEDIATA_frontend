            ...
            <ul className={NavbarStyles.navlinks}>
                <TransitionGroup component={null}>
                    {isAuthenticated && selectedNode && (
                        <CSSTransition key="extra-options" timeout={300} classNames={{
                            ...
                            <div className={NavbarStyles.darkerContainer}>
                                <li className={`${NavbarStyles.listItem} ${NavbarStyles.darkerItem}`}>
                                    <CustomLink to="/csvchecker" onClick={toggleMenu} noToggle>CSV Checker</CustomLink>
                                </li>
                                <li className={`${NavbarStyles.listItem} ${NavbarStyles.darkerItem}`}>
                ...
                <input type="checkbox" id="menuToggle" className={NavbarStyles.checkboxToggle} checked={menuOpen} onChange={toggleMenu} />
                ...
                <div className={`${NavbarStyles.menu} ${menuOpen && animate ? NavbarStyles.animate : ''}`}>
                    ...
                    {isAuthenticated && (
                        <li className={NavbarStyles.listItem}>
                            <CustomLink to="/nodes" onClick={toggleMenu}>Nodes</CustomLink>
                        </li>
                    )}
                    {!isAuthenticated ? (
                        <li className={NavbarStyles.listItem}>
                            <CustomLink to="/login" onClick={toggleMenu}>Login</CustomLink>
                        </li>
                    ) : (
                        <li className={NavbarStyles.listItem}>
