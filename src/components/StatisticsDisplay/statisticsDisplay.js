  ...
  ({ data, showOutliers, setSelectedEntry, selectedEntry }) => {
    ...
    const categoricalDataForTable = useMemo(
      () => ({
        ...
        "Total Count": data.categoricalFeatures.map((feature) =>
          feature.count.toString()
        ),
        Mode: data.categoricalFeatures.map((feature) => feature.mode || "N/A"),
        "Mode Frequency": data.categoricalFeatures.map((feature) =>
          feature.modeFrequency ? feature.modeFrequency.toString() : "N/A"
        ),
        "Mode %": data.categoricalFeatures.map((feature) =>
          feature.modeFrequencyPercentage
            ? `${feature.modeFrequencyPercentage.toFixed(2)}%`
            : "N/A"
        ),
        "2nd Mode": data.categoricalFeatures.map(
          (feature) => feature.secondMode || "N/A"
        ),
        "2nd Mode Frequency": data.categoricalFeatures.map((feature) =>
          feature.secondModeFrequency
            ? feature.secondModeFrequency.toString()
            : "N/A"
        ),
        "2nd Mode %": data.categoricalFeatures.map((feature) =>
          feature.secondModePercentage
            ? `${feature.secondModePercentage.toFixed(2)}%`
            : "N/A"
        ),
        "Missing Entries": data.categoricalFeatures.map((feature) =>
          formatMissingEntries(feature.count, feature.missingValuesCount)
        ),
      }),
      [data]
