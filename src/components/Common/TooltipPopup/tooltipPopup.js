...
const TooltipPopup = ({ message, buttonRef, onClose, offsetY = 15 }) => {
  const [show, setShow] = useState(false);
  const [position, setPosition] = useState({ top: 0, left: 0 });
  const [width, setWidth] = useState(null);
  const [clipPath, setClipPath] = useState(
    `polygon(0% 0%, 100% 0%, 100% 80%, 55% 80%, 50% 100%, 45% 80%, 0% 80%)`
  );
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      if (buttonRef.current) {
        ...
        let top = buttonRect.top - offsetY + window.scrollY - buttonRect.height;
        ...
        if (left + width / 2 > window.innerWidth) {
          ...
          setClipPath(
            `polygon(0% 0%, 100% 0%, 100% 80%, 90% 80%, 85% 100%, 80% 80%, 0% 80%)`
          );
        } else if (left - width / 2 < 0) {
          left = width / 2 + 10;
          setClipPath(
            `polygon(0% 0%, 100% 0%, 100% 80%, 10% 80%, 15% 100%, 20% 80%, 0% 80%)`
          );
        }
  ...
  }, [buttonRef, onClose, width, offsetY]);
  ...
  const tooltipElement = (
    <CSSTransition
      ...
      unmountOnExit
      onEntering={() => {
        const currentWidth = tooltipRef.current
