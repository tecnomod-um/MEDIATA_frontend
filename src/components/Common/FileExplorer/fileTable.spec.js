...
jest.mock("./fileTypeIcon", () => ({ name }) => <div data-testid="file-icon">{name}</div>);
...
describe("<FileTable />", () => {
  const mockCallbacks = {
    ...
    formatDateTime: jest.fn((ms) => new Date(ms).toLocaleDateString()),
    isNew: jest.fn(() => false),
  ...
  it("shows new mark for new files", () => {
    const isNewMock = jest.fn((f) => f.name === "test.csv");
