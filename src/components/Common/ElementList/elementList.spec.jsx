...
describe('ElementList', () => {
  ...
  it('applies activeIndex and activeCategoryIndex classes', () => {
    ...
    expect(secondBtn.className).toMatch(/active/i)
    rerender(
      <ElementList
        items={items}
        activeIndex={1}
    ...
    expect(catBtn.className).toMatch(/activeCategory/i)
  })
  ...
  it('applies builtClasses styling', () => {
    ...
    expect(firstBtn.className).toMatch(/built/i)

    const bananaBtn = screen.getByText('Banana')
    expect(bananaBtn.className).toMatch(/built/i)
  })
