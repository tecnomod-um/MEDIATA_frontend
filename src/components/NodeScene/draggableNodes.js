...
const DraggableNodes = ({ nodes, onNodeClick }) => {
    ...
    const nodeSize = Math.max(viewport.width / Math.max(12, nodes.length + 5), 1.50);
    const descriptionWidth = Math.max(viewport.width * 0.2 / Math.max(4, nodes.length - 1), 3.5);
    const fontSize = Math.max(0.3 * viewport.width / Math.max(12, nodes.length + 5), 0.1);
    ...
    const adjustedDescriptionWidth = Math.max(descriptionWidth, minDescriptionWidth);
    const adjustedFontSize = Math.max(fontSize, minFontSize);
    const descriptionSize = [adjustedDescriptionWidth, 0.5];
    ...
    const constrainPosition = useCallback((position) => {
        ...
        return {
            x: Math.min(Math.max(position.x, -halfWidth + nodeSize / 2), halfWidth - nodeSize / 2),
            y: Math.min(Math.max(position.y, -halfHeight + nodeSize / 2), halfHeight - nodeSize / 2)
        };
    }, [viewport, nodeSize]);
    ...
    return (
        <group ref={groupRef}>
            {nodes.map((node) => (
                <Node
                    key={node.nodeId}
                    node={{ ...node, position: nodePositions[node.nodeId] || node.position }}
