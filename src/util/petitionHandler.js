...
export const nodeAuth = async (serviceUrl, kerberosToken) => {
    ...
    try {
        const response = await nodeAxiosInstance.post(`/taniwha/node/validate`, { kerberosToken });
        if (response.status === 200) {
...
export const uploadFile = async (file, onUploadProgress) => {
    ...
    try {
        const response = await nodeAxiosInstance.post(`/taniwha/api/data/process`, formData, {
            headers: {
...
export const recalculateFeature = async (file, featureName, featureType) => {
    ...
    try {
        const response = await nodeAxiosInstance.post(`/taniwha/api/data/reprocess`, formData, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
